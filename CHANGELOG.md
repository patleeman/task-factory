...
- First-class daemon→gateway notification pipeline for background events. Added typed notification contracts, durable intake queue + worker retries/failure capture, allowlisted Telegram/Discord routing (global defaults + workspace overrides), queue-manager lifecycle emits, and new notification REST endpoints/docs.
- Agent-friendly CLI automation flags: `workspace delete` and `task delete` now support `-y, --yes` (alias `--force`) to bypass interactive confirmation; `workspace list/show`, `task list/show`, and `stats` now support machine-readable output via `--json` or `--output json`.
- New `task-factory capabilities` command returns stable machine-readable JSON with CLI version, command surface, and required-agent compatibility status (`supportLevel: full|partial`) so agents can adapt behavior on older/partial installs.
- `task-factory task create` now supports `-f, --file <path>` for UTF-8 markdown/text task content, with clear non-zero errors for unreadable file paths. When both `--content` and `--file` are provided, file content takes precedence.
- Added `npm run check:cli-drift` release gate to detect drift between shipped CLI command surface and documented Task Factory skill/CLI reference command coverage.
- Added `docs/cli-migration.md` with global upgrade, verification, missing-capability warning, and rollback guidance for users upgrading from older installs (for example 0.3.0).
- Workspace pipeline stats API + Foreman dashboard panel: `GET /api/workspaces/:workspaceId/pipeline-stats` now exposes funnel counts/transitions/conversion rates, rework metrics (including planning/execution model breakdowns with `unknown` fallback), and flow metrics (throughput + cycle/lead avg/median/p95). The Foreman workspace pane now renders these metrics as summary cards, funnel view, and model rework tables when no artifact is selected.
### Changed
...
- Workspace/task slash skill discovery and task-lane skill pickers now use one canonical workspace skill catalog (`GET /api/workspaces/:workspaceId/skills`) that includes source/provider/path attribution plus resolved `enabled` state; external auto-discovered skills are visible but disabled by default unless selected via global/workspace settings.
